...
void setup()
{
  ...
  timeoutCallMotion = timeoutCallMonitor;
...
bool checkWIFIConnection()
{
  ...
  if (rc)
  {
    ...
    // sometimes id cget "0"
    if (strlen(config.macAddress) < 5)
    {
      logger.printlnLog("wrong data from WifiConnection .. disconnect ..");
      wifi.disconnect();
    }
  }
  return rc;
...
void loopMQ()
{
  ...
  if (wifiConnectionAvailable)
  {
    ...
    if (mqttConnectionAvailable)
    {
      ...
      bool checkSendMotionData = (now - timeoutCallMotion) > WAIT_CALL_MOTION;
      digitalWrite(ledPIN, led);
      if (checkSendMotionData || signal != motionStatus)
      {
        sendMotionData(config, sensorData, signal);
        motionStatus = signal;
...
void sendMotionData(CONFIG &cfg, SENSORDATA &sensor, int on)
{
...
void publishMessage(char *message, char *topic)
{
  ...
  if (!rc)
    logger.printlnLog("Publish failed !!");
#endif
#ifdef MQTT_0
